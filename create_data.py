...
def histogram_match_data(base, source, task_id, task_name, spacing, n_samples, input_dim,
    ...
    with tqdm(total=(len(volumes_path) + len(artifacts_path)) * 2, desc=f'original volume numbers') as pbar:
        for vpath, lpath in zip(volumes_path, labels_path):
            ...
            # for dense cubes in NIBS-serotonin use labels but not labels_sk
            # if 800 <= int(case) <= 850:
            #     lpath = join(data_path, "labels", "label-"+case+".tif")
            #     print(lpath)
            ...
            total_ori_volumes += 1
            pbar.update()
            # if total_ori_volumes >= len(volumes_path) / 2:
            #     break
        if match_flag and join_flag:
            for vpath_s, lpath_s in zip(volumes_path_s, labels_path_s):
                ...
                total_ori_volumes += 1
                pbar.update()
                # if total_ori_volumes >= len(volumes_path):
                #     break
        ...
        if data_mix:
            for apath in artifacts_path:
                ...
                total_ori_volumes += 1
                pbar.update()
                # if total_ori_volumes >= aug_axon_num + len(artifacts_path) / 2:
                #     break
            if match_flag and join_flag:
                for apath_s in artifacts_path_s:
                    ...
                    total_ori_volumes += 1
                    pbar.update()
                    # if total_ori_volumes >= aug_axon_num + len(artifacts_path):
                    #     break
    ...
    with tqdm(total=data_len, desc='volumes numbers') as pbar:
        ...
        elif os.path.exists(join(base, "val", "volumes")):
            val_volume_path = join(base, "val", "volumes")
        else:
            val_volume_path = join("/media/root/data4/zyt/train/Renlab/", "val", "volumes")
...
if __name__ == "__main__":
    # NIBS_seretonin | zsl8_dense | Renlab | dn_3brain | NIBS_gaba | gaba_2 | fn |
    base = "/media/root/data4/zyt/train/gaba_2/"
    source = "/media/root/data4/zyt/train/gaba_2/"
    task_id = 7166
    task_name = 'NIBS_GABA_new_dense3_match_cutmix'
    spacing = (1, 0.126, 0.126)
    histogram_match_data(base, source, task_id, task_name, spacing, 2, 128,
                         True, True, True, True, False)  # artifact mix, train, histogram match, cutmix, join(s&t)
